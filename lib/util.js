...
exports.gruntGetFiles = function(task) {
    if (task.indexOf(':') > 0) task = task.replace(':', '.');
...
exports.getDestFileFromConfig = function(srcPath, config) {
    ...
    if (matchedDest !== undefined) matchedDest = matchedDest.dest;
...
exports.findDestFileFromTasks = function(srcPath, tasks) {
    ...
    var task =_.find(tasks, function(t) {
        ...
        if (destPath !== undefined) return true;
    });
 ...
 * @param tasksByParent
 */
exports.populatePipeline = function(pipeline, tasksByParent) {
...
exports.collectPipelineIgnoredFilesByTask = function(pipelines) {
    ...
    return _.reduce(grouped, function(filesByTask, results) {
        _.each(filesByTask, function(task, files) {
            if (results[task] === undefined) results[task] = [];
 ...
 * @return {Array}
 */
exports.extractPipelines = function(tasksByParent) {
    var pipelines = [];
 ...
 * @return {Promise}
 */
exports.downloadFile = function(file, sourceFiles, options) {
...
exports.processTaskFile = function(task, file, options) {
    ...
    var destVersionedPath = exports.addVersionToPath(file.dest, hash);
    ...
    return exports.downloadFile(destVersionedPath, file.src, options);
};
...
exports.processTaskFiles = function(task, options, doneCallback) {
    ...
    var files = grunt.task.normalizeMultiTaskFiles(config);
    files.forEach(function (f) {
        promises.push(exports.processTaskFile(task, f, options));
    });
    ...
    when.all(promises).done(function(filesNotInRepository) {
        ...
        if (filesNotInRepository.length > 0) {
            exports.runProxyTask(task, config, filesNotInRepository);
        }

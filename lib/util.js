...
exports.addVersionedFilesMap = function(mapping, options) {
    ...
    // Normalize the paths to ensure that they are relative to the Gruntfile
    var cwd = process.cwd();
    ['originalPath', 'versionedPath'].forEach(function(p) {
        mapping[p] = path.relative(cwd, mapping[p]);
    });
...
exports.populatePipeline = function(pipeline, tasksByParent, options) {
    var nextParents = tasksByParent._order.slice(1);
 ...
 * @return {Promise}
 */
exports.downloadFile = function(file, dest, context, options) {
    return when.promise(function(resolve, reject) {
        ...
        repository.downloadFile(file, dest, options).done(
            function() {
                grunt.log.writeln('Downloaded "' + file + '" from repository');
                resolve(null);
            },
            function(response) {
                grunt.log.writeln('File "' + file + '" not downloaded from the repository.');
...
exports.processTaskFile = function(task, file, options) {
    ...
    var dest = destVersioned;
    if (options.doNotVersion) {
        dest = file.dest;
    }
    return exports.downloadFile(path.basename(destVersioned), dest, file.src, options);
};
